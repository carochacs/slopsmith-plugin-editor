...
- **Sync detection was unreliable on real stem-only/isolated-instrument files** — validated against real synced and artificially-desynced sloppaks (issue #16 follow-up). Two root causes fixed:
  ...
  - Also added: a **bass-anchored offset for the "sync all arrangements" scope** (bass lines chroma-match far more reliably than dense/sustained guitar — validated: bass recovered a real file's desync to within ~0.15s in both directions, while the guitar-anchored offset was wrong even on an already-synced file). An onset-refinement snap (nudge the DTW result to the nearest detected onset) was also tried but **reverted**: it relies on picking the "nearest" onset to disambiguate, which is an effectively arbitrary tie-break when a real note onset and an incidental noise-driven onset land at nearly equal distance — confirmed on a clean synthetic case where it flipped a correct 0.067s result to a wrong 0.136s one. The raw DTW result is kept.
