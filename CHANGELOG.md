...
### Fixed
- **Sync detection octave-errored on stem-only sloppaks** — beat tracking an isolated melodic stem (all a TabGrabber sloppak carries) reported ~2× the real tempo (152 vs 77 BPM on a Pearl Jam guitar stem), so the client derived a ~2× factor that would halve the whole chart on Apply. `_detect_bpm_and_offset` now seeds `beat_track` with the tab tempo as a prior, folds the result to the octave nearest the tab tempo, snaps a near-unity detected factor to exactly 1, and reports a BPM consistent with the factor actually applied. New regression tests in `tests/test_sync_offset.py`.
- **Sync offset searched the whole decoded region** — subsequence DTW over everything from 0:00 let a far-away recurrence of the opening pitch classes win, producing wild offsets (≈−16s on a lead line whose first note is at 25s). The route now decodes only a local window around the opening notes and `_detect_bpm_and_offset` takes a `load_offset` so the result stays in the tab's absolute timeline. The symbolic pattern also widened to a few opening notes and caps each note's sustain fill, which localizes long sustained first notes.
- **Editor loaded one isolated stem as "the audio"** — a sloppak with no `full` stem (only Demucs stems) played back the first stem (e.g. isolated guitar) instead of the song. The load path now mixes all stems into a full-song track (`ffmpeg amix … normalize=0`, reconstructing the mix without attenuation/clipping) when no `full` stem exists, falling back to a single stem if ffmpeg is unavailable.
